jQuery( function( $ ) {
 ...
 const countryList  = [
    ...
    $(document).ready(function(){
        ...
        alertar()
        // Reemplazado: uso de createSelect en lugar de restcountries
        idleTimeouts();

        // Elementos del DOM donde mostraremos país y bandera
        ...
        // Cargar lista de países: primero intentamos leer JSON embebido en la página
        // (elemento con id "countriesData" cuyo contenido es JSON), luego variable global
        // `EPAYCO_COUNTRIES` o `countriesJson`. Si no hay nada, usamos un array de ejemplo.
        let countries = [];
        try {
            ...
            console.warn('No se pudo parsear #countriesData JSON', e);
            countries = [];
        }
        ...
        if (!countries || !countries.length) {
            if (window.EPAYCO_COUNTRIES && Array.isArray(window.EPAYCO_COUNTRIES)) {
                ...
                // countryList is embedded earlier in the file (JSON with emoji flags)
        ...
        // Detectar idioma de la página (se inyecta en la plantilla como #lang_epayco)
        ...
        // Crea un dropdown personalizado con bandera y nombre de país
        function createSelect(n, selectedCode) {
            ...
            const updateSelected = (idx) => {
                const c = n[idx];
                if (!c) return;
                // Si el idioma de WP es español, preferir name_es; si es inglés, preferir name
                let displayName;
                if (pageLang && pageLang.indexOf('es') === 0) {
                    ...
                    // por defecto, preferir name_es si existe, sino name
                    displayName = c.name_es || c.name;
                }
                if (divSample) {
                ...
                if (divFlag) {
                    // Mantener la clase de flag para conservar estilos existentes
                    const baseClass = 'flag flag-icon-background';
                    divFlag.className = baseClass + ' flag-icon-' + (c.id || '').toLowerCase();

                    // Mostrar bandera: si es URL (flag inicia con http) usamos <img>, si es emoji mostramos un <span>
                    ...
                    // Prefer flagcdn using alpha-2 country code from c.id
            ...
            n.forEach((country, idx) => {
                ...
                a.id = country.id || '';
                // Mostrar bandera y nombre lado a lado (inline)
                ...
                // Mostrar nombre según idioma de la página
                let displayName;
                if (pageLang && pageLang.indexOf('es') === 0) {
                ...
                // bandera: preferimos construir URL hacia flagcdn usando el `id` (alpha-2)
                let flagEl;
                ...
                a.addEventListener('click', function (ev) {
                    ev.preventDefault();
                    ev.stopPropagation(); // evitar que el handler antiguo (`.dropdown dd ul`).click(...) actúe
                    updateSelected(idx);
                    // cerrar dropdown clásico (mantener compatibilidad con handlers existentes)
        ...
        // Inicializar con el país del servidor (elemento #result) o 'CO' por defecto
